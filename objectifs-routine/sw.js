...
const VERSION = "objectifs-routine-v1";
...
// des PWA sœurs (racine, diete-sport/) servies depuis le même
// domaine GitHub Pages.
const PREFIX = "objectifs-routine-v";
const SHELL = [
  "./",
  "./index.html",
  "./style.css",
...
self.addEventListener("fetch", function (event) {
  ...
  if (event.request.method === "POST" && url.pathname.endsWith("/share-target")) {
    event.respondWith((async function () {
      ...
      if (payload) {
        const cache = await caches.open("objectifs-routine-share");
